...
const DepartmentFacultyPaperTable = ({ fPapers, loading, role, dep_id, fetchFacultyPapers }) => {
  ...
  const { isOpen, open, close } = useGroupedByField(fPapers, "researchers");
  const grouped = {};
  fPapers.forEach(paper => {
    paper.researchers.forEach(author => {
      if (!grouped[author]) grouped[author] = { count: 0, rows: [] };
      grouped[author].count++;
      grouped[author].rows.push(paper);
    });
  });
