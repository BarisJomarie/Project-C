...
const DepartmentStudentPaperTable = ({ sPapers, loading, role, dep_id, fetchStudentPapers }) => {
  ...
  const editingPaper = sPapers.find(p => p.research_id === editingPaperId);
  ...
  const saveEdit = async (editingData) => {
    try {
      const res = await axios.put(
        `${API_URL}/api/users/research-student-paper/${editingData.research_id}`, 
        editingData,
  ...
  const handleDeletePaper = (id) => {
    const paper = sPapers.find((p) => p.research_id === id)
      || sPapers.find((p) => p.research_id === id);

    if (!paper) return showToast('error', 'Not Found', 'Paper not found.');

    showModal(
      'Deleting Paper',
      <>Are you sure you want to delete this Student Thesis paper?
        <br/><br/>
          ...
          <table>
            <thead className="stick-header dep-student-thead">
              <tr className="esp-tr student">
                ...
                <th
                  onMouseEnter={() => setHoveredColumn('adviser')}
                  onMouseLeave={() => setHoveredColumn(null)}
                  ...
                  >
                    <div className="filter-inner">
                      <span>Adviser</span>
                      <div className={`filter-arrow ${sortColumn === 'adviser' ? 'active' : ''}`}>
                        {(hoveredColumn === 'adviser' || sortColumn === 'adviser') && ( 
                          <span> 
                            {sortColumn === 'adviser' ? sortDirection === 'asc' 
                            ? <span className="material-symbols-outlined">
                                arrow_upward
                              </span> 
                            : <span className="material-symbols-outlined">
...
export default DepartmentStudentPaperTable;